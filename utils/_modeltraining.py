...
class EmotionalVAE(nn.Module):
    ...
    def __init__(self, latent_dim, hidden_dims, condition_dim=2):
        ...
        self.mu = nn.Linear(hidden_dims[-1], input_dim)
        self.var = nn.Linear(hidden_dims[-1], input_dim)
        
        # Decoder modules
        decoder_layers = []
        
        # Increase latent dimension for emotional conditioning
        decoder_input_dim = input_dim + condition_dim
        
        for i in reversed(range(len(hidden_dims))):
            h_dim = hidden_dims[i]
            
            if i == 0:  # In the last layer, return to the original latent dimension
    ...
    def encode(self, x):
        # x shape: [batch_size, 8, 750]
        # Flatten
        batch_size = x.size(0)
        x_flat = x.view(batch_size, -1)  # [batch_size, 8*750]
    ...
    def train(self, num_epochs=100, max_samples=None):
        ...
        for epoch in range(num_epochs):
            ...
            for batch in progress_bar:
                ...
                # Compute loss
                loss, recon_loss, kl_loss = self.loss_function(recon_batch, latents, mu, logvar)
